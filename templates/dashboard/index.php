...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col">
            <div class="box-item bg-warning">
                <?php
                $crit = 0;
                $med = mysqli_query($conn, "SELECT tbl_medications.Product_ID, tbl_batches.Quantity FROM tbl_batches, tbl_medications WHERE tbl_batches.Product_ID = tbl_medications.Product_ID");
                while($row = mysqli_fetch_array($med))
                {
                    ...
                    if($row["Quantity"] <= $psl["Par_Stock_Level"])
                    {
                        $crit++;
                    }
                }
    ...
    <div class="row">
        ...
        <div class="col-md-8" style="background-color: #fff; border: 1px solid #ccc; border-radius: 20px; box-shadow: 3px 4px 4px #ccc;">
            ...
            <script>
            var xValues = ["Jan 2022","Feb 2022","Mar 2022","Apr 2022","May 2022","Jun 2022","Jul 2022"];
            var yValues = [<?php
                for($i = 1; $i <= 7; $i++)
                {
                    $query = mysqli_query($conn, "SELECT * FROM tbl_appointment WHERE Status = 'approved' AND Date LIKE '2022-0$i%'");
